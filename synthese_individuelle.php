...
echo '</br><h2><em>SYNTHESE INDIVIDUELLE</em>
...
HlpForm::Start(array('url' =>'synthese_individuelle.php','action'=>'get','titre'=>'Choisir Période'));
...
try
{
	...
	$req = 'SELECT * FROM Evenement A, Agent_Evenement B WHERE A.ID_Evenement=B.ID_Evenement AND Date LIKE "'.$periode.'%" AND ID_Agent = '.$_GET['ID_Agent'];
	$st = $connexion->query($req);
	...
	if($donnees == FALSE){
		...
		echo "Aucun évènement ce mois-ci";
			...
			// ... sinon on passe au traitement des données de l'agent par la fonction Tableau (définie dans le fichiers fonctions_heures.php)
			Tableau (array($_GET['ID_Agent']));
