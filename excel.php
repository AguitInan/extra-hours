...
HlpForm::Start(array('url' =>'excel.php','action'=>'get','titre'=>'Choisir Période'));
...
try
{
	...
	if($donnees == FALSE){
		...
		// ... sinon on passe au traitement des données par la fonction Excel (définie dans le fichiers fonctions_heures.php)
		$excel = Excel ($id);
...
// On affiche le bouton d'exportation des données en fichier .xls si on a trouvé des évènements pour le mois sélectionné
if(isset($excel)){

	HlpForm::Start(array('url' =>'exec_query.php','action'=>'post','titre'=>''));
	
	// On sérialise le tableau $excel pour l'envoyer via le formulaire
	echo " <input type='hidden' name='excel' value='".serialize($excel)."'>";
	echo '<div class="form-horizontal">
		<input type="submit" class="btn btn-primary" value="Exporter en fichier Excel">
	  </div>';
